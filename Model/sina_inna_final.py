...
BATCH_SIZE = 256        # Optimized for RTX 4070 Super
NUM_WORKERS = 8         # Optimized for Ryzen 9 7900
...
print(f"--- NIGHT RUN CONFIGURATION ---")
...
def get_loaders(dataset_name, batch_size):
    ...
    testloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=NUM_WORKERS, pin_memory=True, drop_last=False)
...
def train_inna_instance(device, epochs, dataset, alpha, beta, lr=0.05, decay=0.5):
    # clear compiler cache on start of every trainig run
    torch._dynamo.reset()
    ...
    # Removed AMP/Scaler for stability
    ...
    for ep in range(epochs):
        ...
        for x, y in train_l:
            ...
            # Standard Forward/Backward (FP32)
        ...
        if (ep+1)%5 == 0: print(f"    INNA Ep {ep+1}: Acc={acc:.4f}")
            
    return acc_hist, loss_hist

def train_sina_instance(device, epochs, dataset, alpha, beta, gamma0, eps0, decay_rate, decaypower=0.5, prune=False):
    ...
    # COMPILE FOR SPEED
    model = torch.compile(model)
    # Removed AMP/Scaler for stability (Fixes RuntimeError)
    ...
    for ep in range(epochs):
        ...
        for x, y in train_l:
            ...
            # Standard SINA Step (Compiled)
            sina_step(model, state, gamma_k)
            epoch_loss += loss.item()
            
        acc = evaluate(model, test_l, device, eps=state["eps"])
        acc_hist.append(acc)
        loss_hist.append(epoch_loss / len(train_l))
        
        if prune and (np.isnan(acc) or acc < 0.10): return acc_hist, loss_hist, eps_hist
        if (ep+1)%5 == 0: print(f"    SINA Ep {ep+1}: Acc={acc:.4f} (eps={state['eps']:.4f})")
